...
为每个 girl 直接求解合法课表，每门课只受自身约束：
  ① 每人 0 时间冲突（同格同周不双课）
  ② 类型时段：实验=仅下午/晚间(slot5-9,10)；理论=白天(slot1-6)；选修=晚间(slot7-9,10)；讲座=下午
...
def slots_of(c, slot):
    ...
    晚间 slot7-10 = 45分钟/节 → 2小节=2学时；四史3小节=3学时；实验4小节=4学时"""
    if c["type"] in ("体育", "讲座"):
        return [slot]
    ...
    if c["type"] == "实验" and c.get("hps", 2) == 4:   # 4学时 = 晚间4小节（slot7-8-9-10）
        return [7, 8, 9, 10] if slot == 7 else [slot]
...
class Scheduler:
    ...
    def slot_cands(self, c):
        t = c["type"]
        if t == "实验":
            return [7]                # 4学时一次：晚间4小节(slot7-8-9-10)连续
        if t == "理论":
